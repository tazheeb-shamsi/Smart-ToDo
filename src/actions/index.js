...
export function getTodos(date) {
  // Getting the requested date
  const time = new Date(date);

  //! Separating day, month and year of the requested date
  const day = time.getDate();
  const month = time.getMonth() + 1;
  const year = time.getFullYear();

  //! Changing the requested format to standard date format
...
//! Getting the active days with at least one task in them
